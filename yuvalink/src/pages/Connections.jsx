...
import { Users, UserCheck, MessageSquare, Search, Sparkles, UserMinus, Loader } from "lucide-react";
  ...
  return (
    <div className="animate-fade-in" style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div className="network-tabs">
        <button onClick={() => setActiveTab("connections")} className="network-tab" style={{ color: activeTab === "connections" ? "var(--primary)" : "var(--text-secondary)", borderBottom: activeTab === "connections" ? "2px solid var(--primary)" : "2px solid transparent" }}>
      ...
      <div className="glass-card" style={{ padding: "16px" }}>
        <div style={{ position: "relative", width: "100%" }}>
          ...
          <input type="text" placeholder="Search connections by name, college, or role..." className="glass-input" style={{ width: "100%", paddingLeft: "48px" }} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
      </div>

      {loading ? (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "60px", gap: "10px", color: "var(--text-secondary)" }}>
          <Loader size={20} className="animate-spin" /><span>Loading connections...</span>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "24px" }}>
          {filteredConnections.map(conn => (
            <div key={conn.id} className="glass-card" style={{ padding: "24px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", position: "relative" }}>
              <button onClick={() => handleDisconnect(conn.id, conn.name)} style={{ position: "absolute", top: "12px", right: "12px", background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", width: "32px", height: "32px", borderRadius: "8px" }} className="btn-icon" title="Remove Connection" aria-label="Remove Connection">
